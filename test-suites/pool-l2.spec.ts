...
makeSuite('Pool: L2 functions', (testEnv: TestEnv) => {
  ...
  it('Supply with permit test', async () => {
    ...
    const userBalance = await hDai.balanceOf(deployer.address);
    expect(userBalance).to.be.eq(amount, 'invalid amount deposited');
  });
  ...
  it('Repay some with hTokens', async () => {
    const {
      deployer,
      usdc,
      hUsdc,
    ...
    expect(await hUsdc.connect(user1.signer).transfer(deployer.address, repayAmount));

    const balanceBefore = await usdc.balanceOf(deployer.address);
    const debtBefore = await vDebtToken.balanceOf(deployer.address);

    const encoded = await encoder.encodeRepayWithHTokensParams(
      usdc.address,
      repayAmount,
      RateMode.Variable
    );

    expect(await l2Pool.connect(deployer.signer)['repayWithHTokens(bytes32)'](encoded))
      .to.emit(l2Pool, 'Repay')
      .withArgs(usdc.address, deployer.address, deployer.address, repayAmount, true);

    const userDebt = await vDebtToken.balanceOf(deployer.address);
    const userBalance = await usdc.balanceOf(deployer.address);
    const userABalance = await hUsdc.balanceOf(deployer.address);
    expect(userDebt).to.be.eq(debtBefore.sub(repayAmount), 'invalid amount repaid');
  ...
  it('Withdraw some', async () => {
    ...
    const balanceBefore = await hDai.balanceOf(user0.address);
  ...
  it('liquidationCall max value', async () => {
    ...
    const liquidatorAUSDCBefore = await hUsdc.balanceOf(liquidator.address);
    ...
    expect(await hUsdc.balanceOf(liquidator.address)).to.be.gt(liquidatorAUSDCBefore);
    expect(
