...
makeSuite('Upgradeability', (testEnv: TestEnv) => {
  ...
  context('PoolConfigurator upgrade ability', () => {
    ...
    let newHTokenAddress: string;
    let newStableTokenAddress: string;
    let newVariableTokenAddress: string;

    before('deploying instances', async () => {
      const { dai, pool } = testEnv;
      const hTokenInstance = await deployMockHToken([
        pool.address,
        dai.address,
        ZERO_ADDRESS,
      ...
      newHTokenAddress = hTokenInstance.address;
    ...
    it('Tries to update the DAI Htoken implementation with a different address than the poolManager', async () => {
      ...
      const symbol = await (await getHToken(newHTokenAddress)).symbol();
      ...
      await expect(
        configurator.connect(users[1].signer).updateHToken(updateHTokenInputParams)
    ...
    it('Upgrades the DAI Htoken implementation ', async () => {
      ...
      await configurator.updateHToken(updateHTokenInputParams);

      const tokenName = await hDai.name();

      expect(tokenName).to.be.eq('Hope Interest bearing DAI updated', 'Invalid token name');
    });
    ...
    it('Upgrades the DAI stable debt token implementation ', async () => {
      ...
      expect(tokenName).to.be.eq('Hope stable debt bearing DAI updated', 'Invalid token name');
    });
    ...
    it('Upgrades the DAI variable debt token implementation ', async () => {
      ...
      expect(tokenName).to.be.eq('Hope variable debt bearing DAI updated', 'Invalid token name');
    });
  });
});
