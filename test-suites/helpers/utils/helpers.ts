...
export const getReserveData = async (
  ...
  const hToken = (await getHToken(tokenAddresses.hTokenAddress)) as HToken;

  // Need the reserve factor
  const reserveFactor = reserveConfiguration.reserveFactor;

  const availableLiquidity = await token.balanceOf(hToken.address);
  ...
  return {
    ...
    address: reserve,
    hTokenAddress: tokenAddresses.hTokenAddress,
    symbol,
    decimals,
...
export const getUserData = async (
  ...
  const [userData, scaledHTokenBalance] = await Promise.all([
    helper.getUserReserveData(reserve, user),
    getHTokenUserData(reserve, user, helper),
  ]);
  ...
  return {
    scaledHTokenBalance: BigNumber.from(scaledHTokenBalance),
    currentHTokenBalance: userData.currentHTokenBalance,
...
const getHTokenUserData = async (
  reserve: string,
  user: string,
  helpersContract: HopeLendProtocolDataProvider
) => {
  const hTokenAddress: string = (await helpersContract.getReserveTokensAddresses(reserve))
    .hTokenAddress;

  const hToken = await getHToken(hTokenAddress);

  const scaledBalance = await hToken.scaledBalanceOf(user);
