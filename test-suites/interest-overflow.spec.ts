...
import { evmSnapshot, evmRevert, increaseTime } from '@hopelend/deploy';
...
makeSuite('Interest Rate and Index Overflow', (testEnv) => {
  ...
  before(async () => {
    const { pool, poolAdmin, configurator, dai, helpersContract, addressesProvider, deployer } =
      testEnv;
  ...
  it('ReserveLogic `cumulateToLiquidityIndex` with liquidityIndex > type(uint128).max (revert expected)', async () => {
    ...
    await dai.connect(user.signer).transfer(hDai.address, toBorrow);

    const mockFlashLoan = await new MockFlashLoanReceiver__factory(await getFirstSigner()).deploy(
      addressesProvider.address
    );
    await dai.addMinter(mockFlashLoan.address);
