...
makeSuite('Pool Liquidation: Edge cases', (testEnv: TestEnv) => {
  ...
  it('Liquidate the whole WETH collateral with 10% liquidation fee, asset should not be set as collateralized anymore', async () => {
    const { pool, users, dai, usdc, weth, hWETH, oracle, configurator } = testEnv;
    ...
    const hWETHToken = HToken__factory.connect(wethData.hTokenAddress, depositor.signer);

    expect(await hWETHToken.balanceOf(borrower.address)).to.be.gt(0);
    ...
    expect(await hWETHToken.balanceOf(borrower.address)).to.be.eq(0);
