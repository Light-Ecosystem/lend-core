...
makeSuite('Pool: Simple FlashLoan', (testEnv: TestEnv) => {
  ...
  it('Takes simple WETH flash loan and returns the funds correctly', async () => {
    const { pool, helpersContract, weth, hWETH } = testEnv;
  ...
  it('Takes out a 500 USDC flashloan, returns the funds correctly', async () => {
    const { usdc, hUsdc, pool, helpersContract, deployer: depositor } = testEnv;
    ...
    const reservesBefore = await hUsdc.balanceOf(await hUsdc.RESERVE_TREASURY_ADDRESS());
    ...
    const reservesAfter = await hUsdc.balanceOf(await hUsdc.RESERVE_TREASURY_ADDRESS());
  ...
  it('Check that reentrance borrow within flashloanSimple impacts rates', async () => {
    ...
    const availableBefore = await dai.balanceOf(hDai.address);
    ...
    const availableAfter = await dai.balanceOf(hDai.address);
