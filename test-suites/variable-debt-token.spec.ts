...
import { evmRevert, evmSnapshot, getVariableDebtToken, increaseTime, waitForTx } from '@hopelend/deploy';
...
makeSuite('VariableDebtToken', (testEnv: TestEnv) => {
  ...
  it('User borrows and repays in same block with zero fees', async () => {
    const { pool, users, dai, hDai, usdc, variableDebtDai } = testEnv;
    const user = users[0];
    ...
    expect(await dai.balanceOf(hDai.address)).to.be.eq(utils.parseEther('2000'));
  ...
  it('User borrows and repays in same block using credit delegation with zero fees', async () => {
    ...
    expect(await dai.balanceOf(hDai.address)).to.be.eq(utils.parseEther('1000'));
