...
import { getMockFlashLoanReceiver } from '@hopelend/deploy/dist/helpers/contract-getters';
...
makeSuite('PausableReserve', (testEnv: TestEnv) => {
  ...
  it('User 0 deposits 1000 DAI. Configurator pauses pool. Transfers to user 1 reverts. Configurator unpauses the network and next transfer succeeds', async () => {
    const { users, pool, dai, hDai, configurator } = testEnv;
    ...
    const user1Balance = await hDai.balanceOf(users[1].address);
    ...
    await expect(
      hDai.connect(users[0].signer).transfer(users[1].address, amountDAItoDeposit)
    ).to.revertedWith(RESERVE_PAUSED);

    const pausedFromBalance = await hDai.balanceOf(users[0].address);
    const pausedToBalance = await hDai.balanceOf(users[1].address);
    ...
    await hDai.connect(users[0].signer).transfer(users[1].address, amountDAItoDeposit);
