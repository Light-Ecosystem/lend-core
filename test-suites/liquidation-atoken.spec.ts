...
makeSuite('Pool Liquidation: Liquidator receiving hToken', (testEnv) => {
  const {
  ...
  it('Liquidates the borrow', async () => {
    ...
    expect(expectedCollateralLiquidated).to.be.closeTo(
      userWethReserveDataBefore.currentHTokenBalance.sub(
        userWethReserveDataAfter.currentHTokenBalance
      ),
      2,
    ...
    await expect(tx)
      .to.emit(MockHTokenRepayment__factory.connect(hDai.address, borrower.signer), 'MockRepayment')
