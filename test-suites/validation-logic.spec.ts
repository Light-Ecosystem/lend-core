...
import {waitForTx, evmSnapshot, evmRevert} from '@hopelend/deploy';
...
makeSuite('ValidationLogic: Edge cases', (testEnv: TestEnv) => {
  ...
  it('validateBorrow() when reserve is not active (revert expected)', async () => {
     ...
     * Can be done by sending dai directly to hDai contract after it have been deactivated.
     * If deposited normally it is not possible for us deactivate.
     */

    const {pool, poolAdmin, configurator, helpersContract, users, dai, hDai, usdc} = testEnv;
    const user = users[0];
    ...
    // Transferring directly into hDai such that we can borrow
    await dai.connect(user.signer)['mint(uint256)'](utils.parseEther('1000'));
    await dai.connect(user.signer).transfer(hDai.address, utils.parseEther('1000'));
  ...
  it('validateBorrow() stable borrowing where collateral is mostly the same currency is borrowing (revert expected)', async () => {
    ...
    // ltv != 0
    // amount < hToken Balance

    const {pool, users, dai, hDai, usdc} = testEnv;
    const user = users[0];
  ...
  it('validateRepay() the variable debt when is 0 (stableDebt > 0) (revert expected)', async () => {
    ...
    await dai.connect(user.signer)['mint(uint256)'](utils.parseEther('2000'));
    await dai.connect(user.signer).transfer(hDai.address, utils.parseEther('2000'));
  ...
  it('validateSwapRateMode() when reserve is not active', async () => {
    ...
    const {pool, poolAdmin, configurator, helpersContract, users, dai, hDai} = testEnv;
    const user = users[0];
  ...
  it('validateSwapRateMode() where collateral is mostly the same currency is borrowing (revert expected)', async () => {
    ...
    // isUsingAsCollateral == true
    // ltv != 0
    // stableDebt + variableDebt < hToken
  ...
  it('validateSetUseReserveAsCollateral() when reserve is not active (revert expected)', async () => {
     ...
     * hToken balance (aDAI) its not technically possible to end up in this situation.
     * However, we impersonate the Pool to get some aDAI and make the test possible
     */
    const {pool, configurator, helpersContract, poolAdmin, users, dai, hDai} = testEnv;
    const user = users[0];
    ...
    expect(await hDai.connect(poolSigner).mint(user.address, user.address, 1, 1));
