...
makeSuite('HToken: Mint and Burn Event Accounting', (testEnv) => {
  ...
  before('User 0 deposits 100 DAI, user 1 deposits 1 WETH, borrows 50 DAI', async () => {
    const { dai, configurator, hDai, deployer, pool } = testEnv;
  ...
  it('User 1 supplies DAI', async () => {
    ...
    const hDaiBalance = await hDai.balanceOf(depositor.address);
    expect(hDaiBalance).to.be.equal(firstDaiDeposit);
  });
  ...
  it('User 1 supplies DAI on behalf of user 2', async () => {
    ...
    const hDaiBalance = await hDai.balanceOf(receiver.address);
    expect(hDaiBalance).to.be.equal(firstDaiDeposit);
  });
  ...
  it('User 2 supplies ETH,and borrows DAI', async () => {
    ...
    expect(borrowerWethData.currentHTokenBalance).to.be.equal(amountETHtoDeposit);
  ...
  it('User 1 - supplies more DAI - confirm mint event includes accrued interest', async () => {
    ...
    const hDaiBalance = await hDai.balanceOf(depositor.address);
    accruedInterest1 = hDaiBalance.sub(firstDaiDeposit).sub(secondDaiDeposit);
    ...
    expect(rawTransferEvents.length).to.equal(2, 'Incorrect number of Transfer Events');
    const parsedTransferEvent = hDai.interface.parseLog(rawTransferEvents[1]);

    // get mint event
    const parsedMintEvents = getHTokenEvent(hDai, depositTx, 'Mint');
    expect(parsedMintEvents.length).to.equal(1, 'Incorrect number of Mint Events');
    const parsedMintEvent = parsedMintEvents[0];
  ...
  it('User 1 supplies more DAI again - confirm mint event includes accrued interest', async () => {
    ...
    // get mint event
    const parsedMintEvents = getHTokenEvent(hDai, depositReceipt, 'Mint');
    expect(parsedMintEvents.length).to.equal(1, 'Incorrect number of Mint Events');
    const parsedMintEvent = parsedMintEvents[0];
  ...
  it('User 2 repays all remaining DAI', async () => {
    ...
    await expect(repayTx)
      .to.emit(hTokenRepayImpl.attach(hDai.address), 'MockRepayment')
  ...
  it('User 1 withdraws all deposited funds and interest', async () => {
    ...
    const hDaiBalance = await hDai.balanceOf(depositor.address);
    expect(hDaiBalance).to.be.equal(0);
    ...
    expect(rawTransferEvents.length).to.equal(2, 'Incorrect number of Transfer Events');
    const parsedTransferEvent = hDai.interface.parseLog(rawTransferEvents[0]);

    // get burn event
    const parsedBurnEvents = getHTokenEvent(hDai, withdrawReceipt, 'Burn');
    expect(parsedBurnEvents.length).to.equal(1, 'Incorrect number of Burn Events');
    const parsedBurnEvent = parsedBurnEvents[0];
  ...
  it('User 1 withdraws amount less than accrued interest', async () => {
    ...
    const hTokenSupplyAfter = await hDai.balanceOf(depositor.address);
    const daiReserveData = await helpersContract.getReserveData(dai.address);
    const totalMinted = hTokenSupplyAfter.sub(firstDaiDeposit);
    ...
    // get mint event
    const parsedMintEvents = getHTokenEvent(hDai, withdrawReceipt, 'Mint');
    expect(parsedMintEvents.length).to.equal(1, 'Incorrect number of Mint Events');
    const parsedMintEvent = parsedMintEvents[0];
