...
makeSuite('Pool: Edge cases', (testEnv: TestEnv) => {
  ...
  it('Tries to call `finalizeTransfer()` by a non-hToken address (revert expected)', async () => {
  ...
  it('Initialize an already initialized reserve. ReserveLogic `init` where hTokenAddress != ZERO_ADDRESS (revert expected)', async () => {
    ...
    await expect(
      pool.connect(configSigner).initReserve(
        dai.address,
        config.hTokenAddress, // just need a non-used reserve token
        config.stableDebtTokenAddress,
        config.variableDebtTokenAddress,
        ZERO_ADDRESS
      )
  ...
  it('Init reserve with ZERO_ADDRESS as hToken twice, to enter `_addReserveToList()` already added (revert expected)', async () => {
    /**
     * To get into this case, we need to init a reserve with `hTokenAddress = address(0)` twice.
     * `_addReserveToList()` is called from `initReserve`. However, in `initReserve` we run `init` before the `_addReserveToList()`,
     * and in `init` we are checking if `hTokenAddress == address(0)`, so to bypass that we need this odd init.
     */
    const { pool, dai, deployer, configurator } = testEnv;
  ...
  it('Add asset after multiple drops', async () => {
    ...
    const hTokenImplementation = await new HToken__factory(await getFirstSigner()).deploy(
      pool.address
    );
    ...
    const initInputParams: {
      ...
      {
        hTokenImpl: hTokenImplementation.address,
  ...
  it('Initialize reserves until max-1, then (drop one and add a new) x 2, finally add to hit max', async () => {
    ...
    for (let dropped = 0; dropped < 2; dropped++) {
      ...
      for (let i = 0; i < reservesListBefore.length; i++) {
        ...
        if (assetData.hTokenAddress == ZERO_ADDRESS) {
          continue;
        }
  ...
  it('Tries to initialize a reserve with an HToken, StableDebtToken, and VariableDebt each deployed with the wrong pool address (revert expected)', async () => {
    ...
    const hTokenImp = await new HToken__factory(await getFirstSigner()).deploy(pool.address);
    ...
    const hTokenWrongPool = await new HToken__factory(await getFirstSigner()).deploy(
    ...
    initInputParams[0].hTokenImpl = hTokenWrongPool.address;
    await expect(configurator.initReserves(initInputParams)).to.be.reverted;

    initInputParams[0].hTokenImpl = hTokenImp.address;
  ...
  it('dropReserve(). Only allows to drop a reserve if both the hToken supply and accruedToTreasury are 0', async () => {
    const {
      configurator,
      pool,
      weth,
    ...
    const collectorAddress = await hWETH.RESERVE_TREASURY_ADDRESS();
  ...
  it('validateSupply(). Only allows to supply if amount + (scaled hToken supply + accruedToTreasury) <= supplyCap', async () => {
    const {
      configurator,
      pool,
      weth,
  ...
  it('_checkNoSuppliers() (PoolConfigurator). Properly disables actions if hToken supply == 0, but accruedToTreasury != 0', async () => {
    const {
      configurator,
      pool,
      weth,
