...
makeSuite('LTV validation', (testEnv: TestEnv) => {
  ...
  it('Withdraws DAI', async () => {
    ...
    const hDaiBalanceBefore = await hDai.balanceOf(user1.address);
    ...
    const hDaiBalanceAfter = await hDai.balanceOf(user1.address);

    expect(hDaiBalanceAfter).to.be.eq(hDaiBalanceBefore.sub(withdrawnAmount));
  });
