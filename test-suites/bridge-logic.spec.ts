...
makeSuite('BridgeLogic: Testing with borrows', (testEnv: TestEnv) => {
  ...
  it('User 1 tries to perform fast withdraw 100 hDai from L2 (revert expected)', async () => {
    const { users, pool, dai } = testEnv;
    await expect(
  ...
  it('RiskAdmin updates the unbackedMintCap to 10 hDai (10 left) and user 1 tries to perform fast withdraw 100 hDai from L2 (revert expected)', async () => {
  ...
  it('User 2 perform fast withdraw 100 hDai from L2', async () => {
  ...
  it('RiskAdmin updates the unbackedMintCap to 100 hDai (0 left) and user 1 tries to perform fast withdraw 1 hDai from L2 (revert expected)', async () => {
    const { users, riskAdmin, pool, configurator, dai } = testEnv;
  ...
  it('User 2 perform another fast withdraw 100 hDai from L2', async () => {
    const { users, pool, dai, helpersContract } = testEnv;
    const reserveDataBefore = await getReserveData(helpersContract, dai.address);
    const tx = await waitForTx(
      await pool.connect(users[2].signer).mintUnbacked(dai.address, mintAmount, users[2].address, 0)
  ...
  it('User 2 perform invalid fast withdraw 100 hDai from L2', async () => {
    const { users, pool, dai, helpersContract } = testEnv;
    const reserveDataBefore = await getReserveData(helpersContract, dai.address);
    const tx = await waitForTx(
      await pool.connect(users[2].signer).mintUnbacked(dai.address, mintAmount, users[2].address, 0)
  ...
  it('100 bridged dai used to back unbacked', async () => {
    ...
    const { users, pool, dai, hDai, helpersContract } = testEnv;
  ...
  it('User donates 100 dai to hDai holders', async () => {
    // Let user 3 be bridge for now
    const { users, pool, dai, hDai, helpersContract } = testEnv;
    await dai.connect(users[3].signer)['mint(uint256)'](withdrawAmount);
    await dai.connect(users[3].signer).approve(pool.address, MAX_UINT_AMOUNT);
