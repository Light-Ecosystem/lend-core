...
const getReserveData = async (helpersContract: HopeLendProtocolDataProvider, asset: string) => {
  return Promise.all([
...
makeSuite('PoolConfigurator', (testEnv: TestEnv) => {
  ...
  it('InitReserves via AssetListing admin', async () => {
    const { addressesProvider, configurator, poolAdmin, aclManager, users, pool, deployer } =
      testEnv;
  ...
  it('Check that the reserves have flashloans enabled', async () => {
    const { weth, hope, usdc, dai, helpersContract } = testEnv;
    ...
    const hopeFlashLoanEnabled = await helpersContract.getFlashLoanEnabled(hope.address);
    expect(hopeFlashLoanEnabled).to.be.equal(true);
