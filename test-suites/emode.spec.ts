...
import { evmSnapshot, evmRevert, VariableDebtToken__factory } from '@hopelend/deploy';
...
makeSuite('EfficiencyMode', (testEnv: TestEnv) => {
  ...
  it('User 0 tries to sends hTokens to user 3 (revert expected)', async () => {
    const {
      pool,
      dai,
      hDai,
  ...
  it('User 0 sends hTokens to user 3', async () => {
    const {
      pool,
      dai,
      hDai,
    ...
    const balanceBeforeUser0 = await hDai.balanceOf(user0.address);
    const balanceBeforeUser3 = await hDai.balanceOf(user3.address);

    expect(await hDai.connect(user0.signer).transfer(user3.address, transferAmount));

    expect(await pool.getUserEMode(user0.address)).to.be.eq(CATEGORIES.ETHEREUM.id);
    expect(await pool.getUserEMode(user3.address)).to.be.eq(0);

    expect(await hDai.balanceOf(user0.address)).to.be.eq(balanceBeforeUser0.sub(transferAmount));
    expect(await hDai.balanceOf(user3.address)).to.be.eq(balanceBeforeUser3.add(transferAmount));
  });
